...
json get_overall_network_stats() {
    int bytes_sent = 0, bytes_recvd = 0;
    
    for (const auto& entry : overall_vector) {
        if (entry.bytes < 0) {
            bytes_recvd -= entry.bytes;
        } else {
    ...
    json j;
    j["upload_bytes_per_sec"] = bytes_sent;
    j["download_bytes_per_sec"] = bytes_recvd;
    return j;
}


json get_pidwise_network_stats() {
    json result = json::array();
    
    for (const auto& pair : pid_wise_map) {
        int pid = pair.first.first;
        const std::string& app = pair.first.second;
        ...
        result.push_back({
            {"pid", pid},
            {"app", app},
            {"upload_bytes_per_sec", sent},
            {"download_bytes_per_sec", recvd}
        });
    }
...
json get_appwise_network_stats() {
    std::map<std::string, std::pair<int, int>> app_stats;
    
    for (const auto& entry : pid_wise_map) {
        const std::string& app = entry.first.second;
        
        for (const auto& data : entry.second) {
            if (data.bytes < 0)
            app_stats[app].second -= data.bytes;  // download
            else
            app_stats[app].first += data.bytes;   // upload
        }
    }
    
    json result = json::array();
    for (const auto& app : app_stats) {
        result.push_back({
            {"app", app.first},
            {"upload_bytes_per_sec", app.second.first},
            {"download_bytes_per_sec", app.second.second}
        });
    }
...
int main(){
    ...
    std::thread t_socket(start_network_stats_socket_server, "/tmp/network_stats.sock");
...
void start_network_stats_socket_server(const std::string& socket_path = "/tmp/network_stats.sock") {
    ...
    std::cout << "Socket server listening at: " << socket_path << std::endl;
    ...
    while (true) {
        ...
        if (bytes_read > 0) {
            ...
            json response;
            if (request == "network_overall") {
                response = get_overall_network_stats();
            } else if (request == "network_appwise") {
                response = get_appwise_network_stats();
            } else {
